...
function decompileLogic(code, isPredicate = true) {
    ...
    // Reverse truthy and safety (Dendry Logic artifacts)
    logic = logic.replace(/\s*!==\s*0/g, "");
    logic = logic.replace(/\s*\|\|\s*0/g, "");

    // Q access restoration
    ...
    if (isPredicate) {
        ...
        while (changed) {
            changed = false;
            let next = logic;
            // Strip parens around atoms/names
            next = next.replace(/\(\s*([a-zA-Z_@0-9.]+)\s*\)/g, "$1");
            // Strip parens around comparisons
        ...
        if (logic.startsWith('(') && logic.endsWith(')')) {
            let inner = logic.substring(1, logic.length - 1);
...
module.exports = { extractJsonFromCoreJs, runDecompiler, decompileScene, decompileContent, decompileLogic };
